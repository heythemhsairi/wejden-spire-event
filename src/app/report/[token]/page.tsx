    ...
    <div className="min-h-screen bg-ws-hero">
      ...
      <div className="mx-auto max-w-3xl px-5 py-12">
        <div className="flex items-center justify-between">
          <Badge color="purple">Executive Report</Badge>
        ...
        <h1 className="mt-3 font-display text-3xl font-bold text-ws-ink">
          {row.experience === "cost" && "Hidden Cost Report"}
          {row.experience === "risk" && "Workforce Risk Report"}
          {row.experience === "dashboard" && "Future Workforce Dashboard Report"}
        </h1>
        ...
        {row.summary && (
          <div className="mt-8 rounded-xl border border-ws-border bg-white p-6">
            <h3 className="font-display text-lg font-semibold text-ws-ink">Executive summary</h3>
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          ...
          <Button href="/experience" variant="ghost">Explore more experiences</Button>
        </div>
      </div>
      <SiteFooter />
    </div>
...
function CostReport({ outputs, inputs }: { outputs: Record<string, unknown>; inputs: Record<string, unknown> }) {
  ...
  const rows = [
    ["Turnover cost", Number(outputs.turnoverCost ?? 0)],
    ["Absenteeism cost", Number(outputs.absenteeismCost ?? 0)],
    ["Productivity loss", Number(outputs.productivityLoss ?? 0)],
  ] as const;
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-ws-border bg-gradient-to-br from-ws-cloud to-white p-7">
        <p className="text-xs uppercase tracking-wider text-ws-sage">Estimated annual hidden cost</p>
      ...
      <div className="rounded-xl border border-ws-border bg-white p-6">
        <p className="text-sm text-ws-sage">
          Profile: {String(inputs.employees ?? "—")} employees · {String(inputs.industry ?? "—")} · {String(inputs.turnoverRate ?? "—")}% turnover
        </p>
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-ws-border bg-white p-7 text-center">
        <p className="text-xs uppercase tracking-wider text-ws-sage">Workforce Risk Score</p>
...
function DashboardReport({ outputs, inputs }: { outputs: Record<string, unknown>; inputs: Record<string, unknown> }) {
  ...
  return (
    <div className="rounded-xl border border-ws-border bg-white p-6">
      <p className="text-sm text-ws-sage">Simulated for {String(inputs.industry ?? "—")} · {String(inputs.size ?? "—")} employees</p>
