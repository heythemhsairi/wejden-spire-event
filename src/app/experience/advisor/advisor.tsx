...
const CHIPS = tArr("advisor.chips");
...
export function Advisor() {
  ...
  async function send(text: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.message ?? t("advisor.unavailable"));
  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-4rem)] max-w-3xl flex-col px-5 py-6">
      <div className="mb-4">
        <Badge color="primary">{t("advisor.badge")}</Badge>
        <h1 className="mt-2 font-display text-2xl font-bold text-ws-ink">{t("advisor.title")}</h1>
        <p className="text-sm text-ws-sage">{t("advisor.sub")}</p>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto rounded-xl border border-ws-border bg-ws-cloud p-5">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-xs font-medium uppercase tracking-wider text-ws-text-dim">{t("advisor.suggested")}</p>
        ...
        {messages.map((m, i) => (
          <div key={i} className={m.role === "user" ? "flex justify-end" : "flex justify-start"}>
            <div className={`max-w-[85%] whitespace-pre-wrap rounded-xl px-4 py-3 text-sm leading-relaxed ${m.role === "user" ? "bg-ws-primary text-white" : "border border-ws-border bg-ws-cloud text-ws-ink"}`}>
              {m.content || (streaming && i === messages.length - 1 ? <span className="animate-ws-pulse text-ws-sage">{t("advisor.thinking")}</span> : m.content)}
            </div>
          </div>
        ))}
        {error && (
          <div className="rounded-lg border border-ws-red/30 bg-ws-red/5 px-4 py-3 text-sm text-ws-sage">
            {error}{" "}
            <Link href="/briefing" className="font-medium text-ws-primary underline">{t("advisor.bookInstead")}</Link>
          </div>
        )}
      </div>
      ...
      <p className="mt-2 text-center text-[11px] text-ws-text-dim">
        ...
        <Link href="/briefing" className="text-ws-primary">{t("common.bookBriefing")}</Link>
      </p>
    </div>
  );
}
