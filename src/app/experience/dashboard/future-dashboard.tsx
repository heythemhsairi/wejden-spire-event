  ...
  if (!generated) {
    return (
      <div className="mx-auto flex min-h-[70vh] max-w-xl flex-col justify-center px-5 py-12">
        <Badge color="purple">Experience 03 · Hero</Badge>
        <h1 className="mt-3 font-display text-3xl font-bold text-ws-ink">Generate your CEO dashboard</h1>
        <p className="mt-2 text-ws-sage">Pick your industry and size. We&apos;ll simulate the workforce-risk terminal for an organization like yours.</p>
        ...
        <div className="mt-8 space-y-4 rounded-xl border border-ws-border bg-white p-6">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium uppercase tracking-wider text-ws-sage">Company size</label>
          ...
          <Button variant="primary" className="w-full" onClick={generate} disabled={building}>
            {building ? "Building terminal…" : "Generate ▸"}
          </Button>
        </div>
      </div>
    );
    ...
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-2xl items-center justify-center px-5">
        <div className="text-center">
          ...
          <p className="mt-4 text-ws-sage">Compiling workforce signals…</p>
        </div>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-7xl px-5 py-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-t-xl border border-ws-border bg-ws-cloud px-5 py-3">
        <div className="flex items-center gap-3">
          <span className="font-display text-sm font-bold tracking-tight text-ws-primary">WEJDENSPIRE TERMINAL</span>
          <span className="text-ws-text-dim">·</span>
          <span className="text-sm text-ws-sage">{industry} · {size}</span>
          <Badge color="purple" className="ml-1">simulated</Badge>
        </div>
        <LiveDot />
      </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_300px]">
          <div className="rounded-lg border border-ws-border bg-ws-cloud p-5">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-ws-ink">Organizational risk heatmap</h3>
              <span className="text-[11px] text-ws-text-dim">Burnout exposure · dept × week</span>
            </div>
            <Heatmap data={snap.heatmap} rows={snap.departments} cols={snap.weeks} />
          </div>

          <div className="rounded-lg border border-ws-border bg-ws-cloud p-5">
            <h3 className="mb-3 text-sm font-semibold text-ws-ink">Signal feed</h3>
      ...
      <div className="mt-6 rounded-lg border border-ws-purple/25 bg-ws-soft-purple px-5 py-4 text-center">
        <p className="font-display text-lg font-semibold text-ws-ink">This is what future HR management looks like.</p>
        <p className="mt-1 text-sm text-ws-sage">A simulation today. A daily reality with WejdenSpire.</p>
      </div>

      <div className="mt-5 flex flex-wrap gap-3">
        <Button variant="primary" onClick={() => setLeadOpen(true)}>Get this dashboard for my company →</Button>
        <Button variant="ghost" onClick={() => setGenerated(false)}>Change industry / size</Button>
      </div>

      <LeadModal
        open={leadOpen}
        onClose={() => setLeadOpen(false)}
        title="Get this dashboard for my company"
